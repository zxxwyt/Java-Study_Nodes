...
## 子查询
通俗来讲子查询的本质就是**在一个查询里面，再嵌套一个查询，用前一个查询的结果去当后一个查询的条件或数据来源。**。
下面我将通过标量子查询，列子查询，行组查询以及表子查询彻底讲清楚子查询的实现和思路这一部分也是比较难实现的部分我将详细展开
### 标量子查询
顾名思义，**标量子查询是只返回一个值的子查询，这个值可用来和外层查询中的字段进行比较。**，那么我们看这样一组例子，我们根据之前用到的表格来解决这样一个问题：查询'销售部'的所有员工的信息，在emp的表格当中我们无法找到关于销售部这个字样的详细，但是我们发现销售部这个字样可以通过dept_name等价的找到对应的部门，我们先执行这样的一个语句：
```sql
-- 找到销售部门对应的dept_id
select id from dept where name='销售部';
```
这个查询返回的结果是一个数据id:4。也就是所谓的标量子查询最关键的一步：**找这个表里对应需求的但是需要从其他表格当中才能找到的数据**。我们可以清晰的知道通过这个部门等价的id就能在一张emp的表格里面找到我要的结果：查询'销售部'的所有员工的信息：
我们利用刚刚得到的4
```sql
-- 根据销售部门id查询员工信息
select * from emp where dept_id=4;
```
那么把这两条sql语句合并就是所谓的子查询的第一个分类：标量子查询；
```sql
-- 根据销售部门id查询员工信息
select * from emp where dept_id=(select id from dept where name='销售部');
```
我们发现通过最后两个MySQL语句最后得到的结论是一样的都得到了这个表格，实现了相同的效果：
| id | name   | age | job     | salary | entrydate  | managerid | dept_id |
|----|--------|-----|----------|---------|-------------|------------|----------|
| 14 | 张三丰 | 88  | 销售总监 | 14000   | 2004-10-12  | 1          | 4        |
| 15 | 俞莲舟 | 38  | 销售     | 4600    | 2004-10-12  | 14         | 4        |
...
总结：标量子查询是一种只返回单个值的子查询，常用在 `WHERE`、`HAVING` 或 `SELECT` 子句中，与比较运算符一起使用，用于将子查询返回的结果当作常量参与主查询判断，如比较、计算等操作，是最简单也是最常见的子查询类型。

### 列子查询
顾名思义列子查询返回的是一个列，一句话：**列子查询的本质是返回一列数据供主查询与之进行集合比较（如使用 IN、ANY、ALL 等），用来判断某个字段是否匹配子查询结果集中的任意或全部值。**，我们通过一个例子来说明，比如我们有个这样的需求：查询 "销售部"和“市场部”的所有员工的信息。那么我们可以知道员工的信息在emp的表格里面，但是销售部等的部门信息在dept的表格里面而且**不唯一**，这个时候我们就要通过一个查询找到相关部门名对应的dept_id的信息：
```sql
-- a.查询"销售部"和市场部对应的id
...
没错这个就是列查询当中所说的返回的列，对应了我刚刚所讲述的本质。通过这个列**我们把这个列当作一张表**我们的目的是查询 "销售部"和“市场部”的所有员工的信息。我们已经得到了这些部门对应的dept_id值**他们不是唯一的是以列的形式呈现而且是我们需要的多个数据以一列的形式表现**，接下来就是最后一步：**把这个结果当作表格完成后续的操作**：
```sql
-- a.查询"销售部"和市场部对应的id
-- 查询id等于对应值的员工信息
select * from emp where dept_id in(  select id from dept where name in ('销售部','市场部'));
```
**当然不止涉及到in字段还涉及到all字段**我们来看下面一个例子我们有如下需求：查询比财务部所有人工资都高的员工信息，首先我们需要把财务部的工资都用表格的形式呈现出来，也就是子查询的第一步**先把间接的变量变成直接的**
```sql
-- a.查询”财务部“的每一个人的工资
...
第二步：把这个结果当作表格的形式**直接的形式（salary）**通过all字段关系起来，我们最后做出：
```sql
select * from emp where salary> all (select salary from emp where dept_id=(select id from dept where name='财务部'));
```
这个MySQL的完整的语句。总结一下**列子查询返回一列数据，可供外层查询用来与某个字段进行比较。**
### 行子查询
这个情况和之前的列子查询一样，一句话说明行子查询的本质**列子查询的本质是返回一列值的查询结果集，用于与外层查询的某个字段进行多值比较。**,需要注意一点，行子查询其实是返回的是**一个元组的子集也就是说可能信息不是整个元组而是其中的某几个属性**，我们下面来看一个实际的例子，我们现在有这样一个需求：查询与 "张无忌"的薪资及直属领导相同的员工信息：我们要进行匹配需要一个emp里面某个元组里面的特定的两个属性：薪资和直属领导，而这个刚好是通过行的形式呈现的。我们写出如下的MySQL语句：
```sql
 查询‘张无忌’的薪资及直属领导
...
最后我们把刚刚的这个查询当成是一张表，把这张表里面的元素和整个emp的元组进行匹配：
判断的语句是 **(salary ,managerid)=这张表**所以我们得到如下语句：
```sql
-- 查询与‘张无忌’的薪资及其领导相同的信息
-- 一一对应
...
最后总结一下：****行子查询是一次返回一整行（多个列值）的子查询，常用于与外层查询中的多列进行同时比较。**
### 表子查询
看到标题和我之前讲到的一样，所有的行和列其实就是退化的表格而已我们现在就讲解这个最为普遍和复杂的表子查询，表子查询的本质：**表子查询是返回一个虚拟表的子查询，外层查询可以把它当作一张临时表来继续查询。** 我们执行一下语句完成建表：
```sql
...
这就是表子查询的本质：通过返回的一张表进行匹配操作，找到想要的表格：我们已经知道这两个人物的薪资和工作，我们现在就是找到对应相等的元组于是我们有如下的MySQL语句：
层查询可以把它当作一张临时表来继续查询。** 我们执行一下语句完成建表：
```sql
-- 查询与‘张无忌’的薪资及其领导相同的信息
-- 一一对应
-- b再查询和他们相同的员工信息
...
总结一下：**表子查询是返回一个虚拟表的子查询，外层查询可以把它当作一张临时表来继续查询。**
## 总结
在数据库应用中，多表查询（JOIN）是最常见也是最重要的操作之一。它的作用是将分散在不同表中的数据通过某种关联条件组合在一起，从而获得完整的信息。多表查询的核心在于连接条件，通常是主键和外键之间的等值关系。

根据连接方式的不同，多表查询可以分为几类：内连接（INNER JOIN）、外连接（LEFT JOIN、RIGHT JOIN）、自连接（SELF JOIN） 和 交叉连接（CROSS JOIN）。
内连接只保留两张表中都满足条件的记录；
左外连接保留左表的所有记录，即使右表中没有匹配项；
右外连接则相反，保留右表的所有记录；
自连接用于同一张表自身的关联查询，如员工与上级关系。
编写多表查询时，推荐使用显式连接写法，如：
```sql
SELECT a.name, b.course
FROM student a
JOIN score b ON a.id = b.sid;
```
这种写法结构清晰，方便维护。应注意区分 ON 与 WHERE 的作用：ON 用于连接条件，而 WHERE 用于结果过滤，若把右表条件放在 WHERE 中，可能导致外连接结果被误过滤。
多表查询也可与子查询、聚合函数或分组查询结合使用，实现更复杂的分析需求。例如，先聚合订单金额，再连接客户表，就能统计每位客户的总消费。MySQL 8.0 之后还支持窗口函数，能在多表结果上直接做排名、移动平均等操作，提高查询灵活性。
性能方面，连接列应建立索引，并尽量避免在连接条件上使用函数或类型不一致的列，否则会导致索引失效。对复杂查询可使用 EXPLAIN 分析执行计划，关注连接顺序、访问类型和可能的全表扫描。
总的来说，多表查询是数据库查询的核心能力。掌握不同类型连接的特点，理解 ON 与 WHERE 的差别，合理使用索引与聚合，就能编写出高效、清晰、可维护的 SQL 查询语句，为系统的数据分析和业务逻辑提供坚实支撑。
